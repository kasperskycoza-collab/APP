...
export default function FundGoalModal({ isOpen, onClose, goal }: FundGoalModalProps) {
  const { updateGoalStatus, accounts, currency, themePalette } = useStore();
      ...
      <div className="bg-white dark:bg-slate-800 w-full sm:max-w-md rounded-2xl flex flex-col max-h-[85vh] shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="flex justify-between items-center p-4 border-b border-slate-100 dark:border-slate-800 flex-shrink-0">
          <h2 className="font-bold text-lg text-slate-800 dark:text-slate-100 flex items-center gap-2">
            <PlusCircle className={isUbuntu ? "text-[#E95420]" : "text-emerald-500"} size={20} />
            Fund Goal
          </h2>
          ...
          <form id="fund-goal-form" onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="mb-2">
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Remaining: <span className={`font-bold ${isUbuntu ? 'text-[#E95420]' : 'text-emerald-600'}`}>{formatGoalCurrency(goal.target - goal.current)}</span>
              </p>
            </div>
            ...
            <div>
              ...
              <input
                ...
                className={`w-full border-2 border-slate-200 dark:border-slate-700 rounded-xl p-3 ${isUbuntu ? 'focus:border-[#E95420]' : 'focus:border-emerald-500'} focus:outline-none transition-colors`}
                autoFocus
              />
            </div>
            ...
            <div>
              ...
              <select
                ...
                className={`w-full border-2 border-slate-200 dark:border-slate-700 rounded-xl p-3 ${isUbuntu ? 'focus:border-[#E95420]' : 'focus:border-emerald-500'} focus:outline-none transition-colors appearance-none bg-white dark:bg-slate-800`}
              >
            ...
            <div>
              ...
              <input
                ...
                className={`w-full border-2 border-slate-200 dark:border-slate-700 rounded-xl p-3 ${isUbuntu ? 'focus:border-[#E95420]' : 'focus:border-emerald-500'} focus:outline-none transition-colors text-sm bg-white dark:bg-slate-800`}
              />
            </div>
            ...
            <div className="pt-4 pb-2">
              <button
                ...
                className={`w-full font-bold py-4 rounded-xl text-white shadow-lg transition-transform active:scale-[0.98] ${
                  isUbuntu
                    ? 'bg-[#E95420] hover:bg-[#d4481b] shadow-[#E95420]/30'
